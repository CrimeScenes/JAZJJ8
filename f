...
-- Automatically clear console in an infinite loop
...
if shared.Forbidden.Memory.IsActive then
    local Memory
    ...
    task.spawn(function()
        while task.wait(1) do
            local minMemory = shared.Forbidden.Memory.Params.MinValue
            local maxMemory = shared.Forbidden.Memory.Params.MaxValue
...
-- Update keybind retrieval based on new table structure
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if shared.Forbidden.PointAssist.Aligner then
            ...
            if shared.Forbidden.Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if shared.Forbidden.Silent.mode == "target" then
                    local closestPoint
                    if shared.Forbidden.Silent.TargetMode == "OptimalTargetPoint" then
                        closestPoint = GetOptimalTargetPoint(TargetPlayer.Character)
                    elseif shared.Forbidden.Silent.TargetMode == "Closest Point" then
                        closestPoint = GetClosestPoint(TargetPlayer.Character)
                    elseif shared.Forbidden.Silent.TargetMode == "BasicTargeting" then
                        closestPoint = GetClosestHitPoint(TargetPlayer.Character)
                    end

                    local velocity = GetVelocity(TargetPlayer, "Head")  

                    -- Check if the target's head is visible (wall check)
                    if IsVisible(closestPoint) then
...
local function getDynamicFov(targetPosition)
    ...
    if distance <= shared.Forbidden.Ranges.CloseDistance then
        return shared.Forbidden.Silent.BoudingSizes.CloseSize
    elseif distance <= shared.Forbidden.Ranges.MidDistance then
        return shared.Forbidden.Silent.BoudingSizes.MidSize
    elseif distance <= shared.Forbidden.Ranges.FarDistance then
        return shared.Forbidden.Silent.BoudingSizes.FarSize
    else
        return shared.Forbidden.Silent.BoudingSizes.FarSize  
    end
end
...
local function updateFov(TargetPlayer)
    local settings = shared.Forbidden.Silent.BoudingSizes
    clearFovParts()
...
local function isPartVisible(part)
    if not shared.Forbidden.TargetControl.Safety then 
        return true  
    end
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if velocity.Y < -30 and shared.Forbidden.TargetControl.Aligner then
            shared.Forbidden.Silent.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and shared.Forbidden.TargetControl.Aligner then
            return player.Character:FindFirstChild("Humanoid").MoveDirection * 16
        else
            shared.Forbidden.Silent.Prediction = shared.Forbidden.Silent.Prediction 
            return velocity
        end
    end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[shared.Forbidden.Target.Keybind:upper()] and PointAssist.mode == "hold" then
...
local function IsVisible(targetPosition)
    ...
    local origin = character.Head.Position  -- Camera position (viewer's position)
    local direction = (targetPosition - origin).Unit * 1000  -- Direction vector towards the target

    local rayParams = RaycastParams.new()
    rayParams.FilterType = Enum.RaycastFilterType.Blacklist
    rayParams.FilterDescendantsInstances = {character}  -- Don't raycast through the player's own character

    -- Perform raycasting to check if the target is visible from the camera
    local raycastResult = workspace:Raycast(origin, direction, rayParams)
    
    -- If the ray hits something, check if it's obstructing the target body part
    if raycastResult then
        -- Calculate distance from the raycast hit point to the target position
        return (raycastResult.Position - targetPosition).Magnitude < 5
    end
...
task.spawn(function()
    while task.wait(0.1) do
        if shared.Forbidden.Silent.Enabled then
            Fov.Visible = IsTargeting and shared.Forbidden.Silent.AimSettings.FovSettings.FovVisible  
        end
    end
end)
...
if shared.Forbidden.TargetControl.BlockGroundHits == true then
    local function CheckNoGroundShots(Plr)
        if shared.Forbidden.Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
